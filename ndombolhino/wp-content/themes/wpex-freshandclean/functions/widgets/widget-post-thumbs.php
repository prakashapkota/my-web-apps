...
class wpex_recent_posts_thumb_widget extends WP_Widget {
    ...
    function wpex_recent_posts_thumb_widget() {
        parent::WP_Widget(false, $name = __('WPEX - Featured Posts','wpex') );
    }
								...
								if( has_post_thumbnail() ) {  ?>
									<li class="clr">
										<a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>" class="title">
                                        	<img src="<?php echo aq_resize( wp_get_attachment_url( get_post_thumbnail_id() ), 50, 50, true ); ?>" alt="<?php the_title(); ?>" />
                                        </a>
                                        <a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>">
											<?php
                                            $mytitle = get_the_title();
											if ( strlen($mytitle) > 20 )
											$mytitle = substr($mytitle,0,20) .'...';
											echo $mytitle; ?>
                                        </a>
...
} // class wpex_recent_posts_thumb_widget
