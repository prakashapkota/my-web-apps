    ...
    */
    ...
    App.events = function(){
        ...
        $('.input-options').find('input').on('change', function(){
            ...
            var stores,color,trafic,weather,transit,photos,population;
            stores = color = trafic = weather = transit = photos = population = false;

            $('.input-options').find('input').each(function(index, item){
                if( $(item).prop('checked') ){

                    if($(item).data('show') === "stores"){
                        stores = true;
                    }
            ...
            App.initializeMap(stores,color,trafic,weather,transit,photos,population);
        });
    }
    

    App.initializeMap =  function (stores,color,trafic,weather,transit,photos,population) {
        App.options = {
            stores: stores,
            color: color,
            trafic: trafic,
        ...
        if(App.options.stores){
            App.addStores();
        }
        else{
            App.placeMarker( App.map.getCenter(), defaultPosition);
        }
        if(App.options.trafic){
            App.addTraficLayer();
        }
        if(App.options.weather){
            App.addWeatherLayer();
        }
         if(App.options.transit){
            App.addTransitLayer();
        }
        if(App.options.photos){
           App.addPanoramioLayer(); 
        }
        if(App.options.population){
    ...
    App.addStores = function(){
        for(var i=0; i<stores.length; i++){
            var place = new google.maps.LatLng(stores[i].lat,stores[i].lon);
            App.placeMarker(place, stores[i]);
        }
    }
    ...
    App.addPopulationColorShape = function(){
        for(var i=0; i<stores.length; i++){
            var place = {
                center: new google.maps.LatLng(stores[i][0],stores[i][1]),
                population: stores[i][3]
            }
